...
class Ion_auth
{
	 ...
	 * @author Mathew
	 **/
	public function register($eiin_number, $password, $email, $additional_data = array(), $group_ids = array()) //need to test email activation
	{
		...
		if (!$email_activation)
		{
			$id = $this->ion_auth_model->register($eiin_number, $password, $email, $additional_data, $group_ids);
			if ($id !== FALSE)
			{
	...
	public function logged_in()
	{
		...
		if(!(bool) $this->session->userdata('identity')){
		return (bool) $this->session->userdata('identity_alt') ;	
		}else{
	...
	public function is_cook($id=false)
	{
		$this->ion_auth_model->trigger_events('is_cook');

		$cooks_group = $this->config->item('cooks_group', 'ion_auth');

		return $this->in_group($cooks_group, $id);
	}
