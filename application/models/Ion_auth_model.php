...
class Ion_auth_model extends CI_Model
{
	...
	public function eiin_check($eiin_number = '')
	{
		$this->trigger_events('eiin_check');

		if (empty($eiin_number))
		{
			return FALSE;
		}
		...
		return $this->db->where('eiin_number', $eiin_number)
				->order_by("id", "ASC")
				->limit(1)
		                ->count_all_results($this->tables['users']) > 0;
	}
	 ...
	 * @author Mathew
	 **/
	public function register($eiin_number, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		elseif (($this->identity_column == 'eiin_number' || $this->identity_column_alt == 'eiin_number') && $this->eiin_check($eiin_number))
		{
			$this->set_error('account_creation_duplicate_eiin_number');
			return FALSE;
		}
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$this->db->select($this->identity_column . ', eiin_number, phone, school_name, email, id, password, active, last_login')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc');
	...
	**/
	...
	public function login_remembered_user()
	{
		...
		//get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', id, phone, username, email, last_login')
