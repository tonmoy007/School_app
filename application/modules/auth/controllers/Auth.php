...
class Auth extends CI_Controller {
	...
	function login($from='login')
	{
		...
		if ($this->form_validation->run() == true)
		{
			...
			// echo $from;
			$loggedin=$this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember);
			if ($loggedin)
			{
	...
	function change_password()
	{
		...
		if ($this->form_validation->run() == false)
		{
			...
			//render
			$this->_render_page('index.php/auth/change_password', $this->data);
		}
		else
		{
			...
			{
				$this->session->set_flashdata('message', $this->ion_auth->errors());
				redirect('index.php/auth/change_password', 'refresh');
			}
		}
	}
	// function loginfacebook()
	// {	
	// 	$user=$this->facebook_ion_auth->login();
	//       if(!$user){
	...
	function forgot_password()
	{
		...
		if ($this->form_validation->run() == false)
		{
			...
			$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
			$this->_render_page('index.php/auth/forgot_password', $this->data);
		}
		else
		{
	...
	public function reset_password($code = NULL)
	{
		...
		if ($user)
		{
			...
			if ($this->form_validation->run() == false)
			{
				...
				//render
				$this->_render_page('index.php/auth/reset_password', $this->data);
			}
			else
			{
	...
	function deactivate($id = NULL)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->_render_page('index.php/auth/deactivate_user', $this->data);
		}
		else
		{
	...
	// 	$this->form_validation->set_rules('phone', $this->lang->line('create_user_validation_phone_label','required|is_unique['.$tables['users'].'.phone]'), '');
	...
	function signup(){
		...
		if($this->input->post('submit',true)){
			...
			$phone =$this->input->post('phone');
			$password= $this->input->post('password');
			$names=$this->common->split_name($name);
			$username=$names['first_name'];
			$additional_data = array(
			...
			$data=array();
			if($location){
				$data['from']=($location)?$location:'login';
			}
